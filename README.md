### 👋 Hi, I'm Eduardo (Hdiyja) | Software Engineer

> Software Engineer and CS student at Federico II. Passionate about algorithms, modern C++, and building efficient, end-to-end containerized applications.

---

#### 🛠️ Technical Stack

- **Core Languages:** C++, Java (Spring Boot, JavaFX), TypeScript (React)
  - **Familiar With:** C, Bash, Python, Prolog
- **Infrastructure & Tools:** Docker, Docker Compose, CMake, awk, sed, Valgrind
- **Databases:** PostgreSQL (PL/pgSQL), MariaDB
- **Focus Areas:** Algorithms & Data Structures, Scientific Computing, OOP


#### 🧑🏻‍💻 Experience

| Role | Organization | Domain | Period |
| :--- | :--- | :--- | :--- |
| **Software Developer** | JEFNapoli | IT Consulting | 2025 – Present |
| **Alternanza Scuola-Lavoro** | CREF | Astroparticle Physics | 2017 – 2018 |


#### 🚀 Featured Projects

- 🏢 **[DietiEstates25](https://github.com/CsaProtocol/DietiEstates25)**: Full-stack real estate platform built with *Spring Boot, React, and PostgreSQL*
- ⚡ **[AlgorithmPractice](https://github.com/CsaProtocol/AlgorithmPractice)**: Modern *C++20* implementations of fundamental data structures and algorithms
- 🎬 **[MyVideoteca](https://github.com/CsaProtocol/MyVideoteca)**: Multi-language video management app utilizing *Kotlin* and native components
- 🐢 **[TortoiseHospital](https://github.com/CsaProtocol/TortoiseHospital)**: *JavaFX* care management system designed for national sea turtle recovery centers
- 🌌 **[AstroMusic](https://github.com/CsaProtocol/AstroMusic)**: Award-winning scientific application for the "A Scuola di Astroparticelle" project


#### 🎓 Education

- **B.S. Computer Science** | [University of Naples Federico II](https://informatica.dieti.unina.it/index.php/it/)


#### 📜 Certifications & Courses

- **[AI Fluency Framework & Foundations](https://verify.skilljar.com/c/bpobqx3p3szs)** | Certificate of Completion (apr 2026)
- **[Software Engineering Job Simulation](https://www.theforage.com/completion-certificates/fgHAi6dLhpRsGKyyN/da2T3WZCbMAJD7bNB_fgHAi6dLhpRsGKyyN_NyDJnETQhYHsBuW5t_1767997476098_completion_certificate.pdf)** | Forage & Hewlett Packard Enterprise (gen 2026)
- **[Software Engineering Virtual Experience](https://forage-uploads-prod.s3.amazonaws.com/completion-certificates/Goldman%20Sachs/NPdeQ43o8P9HJmJzg_Goldman%20Sachs_NyDJnETQhYHsBuW5t_1676755690318_completion_certificate.pdf)** | Forage & Goldman Sachs (Feb 2023)
- **[Python: Istruzioni per l'uso](https://www.coursera.org/account/accomplishments/certificate/2VLFJRXASC7J)** | University of Naples Federico II (2020)
- **First Certificate in English** | Cambridge Assessment English (2018)
- **Introduction to Internet of Everything** | Cisco Networking Academy (2018)


#### 🌐 Community & Research

- **Publication:** Work on AstroMusic acknowledged in [*The Voice of the Universe: From- to Alternating School and Work*](https://www.researchgate.net/publication/322478644_The_Voice_of_the_Universe_From-_to_Alternating_School_and_Work)
- **Open Source:** Translated [Jekyll-theme-chirpy](https://github.com/cotes2020/jekyll-theme-chirpy/commit/9a011e14d66195d8b2fb9ec62f3e60a3e56cd032) to Italian
- **Conferences Attended:** FOSDEM ('26), CSL ('24), DevFest Naples ('22), GitHub Universe ('22)

---

*Let's build something great. Feel free to explore my repositories or reach out for collaborations!*
